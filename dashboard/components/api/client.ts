...
export async function fetchLiveProbabilities(matchId: string): Promise<MatchUpdate> {
  const res = await fetch(apiUrl(`/api/v1/live/${encodeURIComponent(matchId)}/probabilities`), { cache: "no-store", headers: withTenantHeaders() })
...
export async function fetchTeamsToPlay(): Promise<TeamsToPlayResponse> {
  const res = await fetch(apiUrl("/api/v1/insights/teams-to-play"), { cache: "no-store", headers: withTenantHeaders() })
...
export async function fetchTrackRecord(championship: string = "all", days: number = 120): Promise<TrackRecordResponse> {
  const res = await fetch(
    ...
    { cache: "no-store", headers: withTenantHeaders() }
  )
...
export async function fetchExplainMatch(matchId: string): Promise<ExplainResponse> {
  const res = await fetch(apiUrl(`/api/v1/explain/match?match_id=${encodeURIComponent(matchId)}`), { cache: "no-store", headers: withTenantHeaders() })
...
export async function fetchMultiMarketConfidence(matchId: string): Promise<MultiMarketConfidenceResponse> {
  const res = await fetch(apiUrl(`/api/v1/insights/multi-market?match_id=${encodeURIComponent(matchId)}`), { cache: "no-store", headers: withTenantHeaders() })
  if (!res.ok) throw new Error(`multi_market_failed:${res.status}`)
  return res.json()
}

export async function fetchUserProfile(): Promise<UserProfile> {
  const res = await fetch(apiUrl("/api/v1/user/profile"), { cache: "no-store", headers: withTenantHeaders() })
...
export async function fetchTenantConfig(): Promise<TenantConfig> {
  const res = await fetch(apiUrl("/api/v1/tenant/config"), { cache: "no-store", headers: withTenantHeaders() })
  if (!res.ok) throw new Error(`tenant_config_failed:${res.status}`)
  return res.json()
}

export async function fetchSystemStatus(): Promise<SystemStatusResponse> {
  const res = await fetch(apiUrl("/api/v1/system/status"), { cache: "no-store", headers: withTenantHeaders() })
