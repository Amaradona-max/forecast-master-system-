...
def _default_territory_path() -> str:
    return os.getenv("TERRITORY_INDEX_PATH") or os.getenv("FORECAST_TERRITORY_INDEX_PATH", "data/team_territory_index.json")
...
def _load_territory(path: str) -> dict[str, Any] | None:
    p = Path(path)
    try:
        st = p.stat()
    except Exception:
...
def get_team_territory(*, championship: str, team: str, territory_path: str | None = None) -> TerritoryLookup | None:
    path = territory_path or _default_territory_path()
    data = _load_territory(path)
    if not isinstance(data, dict):
        return None
    ...
    d = row.get("def_index")
    if not isinstance(off, (int, float)) or not isinstance(d, (int, float)):
        off = row.get("tpx_off")
        d = row.get("tpx_def")
        if not isinstance(off, (int, float)) or not isinstance(d, (int, float)):
            return None
    ...
    meta: dict[str, Any] = {
        "territory_generated_at_utc": data.get("generated_at_utc"),
        "territory_generated_at_unix": data.get("generated_at_unix"),
        "territory_path": path,
        "territory_model": (data.get("meta") if isinstance(data.get("meta"), dict) else {}).get("model"),
        "territory_provider": (data.get("meta") if isinstance(data.get("meta"), dict) else {}).get("provider"),
    }
    n_used = row.get("n_used")
    if isinstance(n_used, (int, float)):
        meta["territory_n_used"] = int(n_used)
    return TerritoryLookup(off_index=float(off), def_index=float(d), meta=meta)
